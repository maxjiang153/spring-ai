...
public class BedrockAnthropic3ChatModel
	...
	protected Message doGetToolResponseMessage(ChatResponse response) {
		...
		if (result == null) {
			return BedrockConverseApiUtils.EMPTY_MESSAGE;
		}
	...
	protected Flux<ChatResponse> doChatCompletionStream(BedrockConverseRequest request) {
		return converseApi.converseStream(request)
			...
			.map(this::processBufferedChatResponses);
	}

	private boolean shouldBeBuffered(ChatResponse response) {
		Generation result = response.getResult();
		if (result == null) {
			return true;
		}
		...
		return metadata.isToolUseEvent();
	}

	private ChatResponse processBufferedChatResponses(List<ChatResponse> responses) {
		if (responses.size() == 1) {
